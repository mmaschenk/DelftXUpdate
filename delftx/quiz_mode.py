...
class Assessments(EventlogProcessor):
    ...
    def __init__(self, course_metadata_map, base_path, connection):
        ...
        for question_id in quiz_question_array:
            ...
            while quiz_question_parent not in block_type_map:
                quiz_question_parent = (
                    course_metadata_map[
                        "child_parent_map"][quiz_question_parent])
            quiz_question_type = block_type_map[quiz_question_parent]
            array = [question_id, quiz_question_type]
            self.quiz_question_record.append(array)

        # Processing events data
        self.submission_event_collection = []
    ...
    def handleEvent(self, jsonObject):
        if jsonObject["event_type"] in self.submission_event_collection:
            ...
            if global_learner_id != "":
                ...
                if question_id != "":
                    ...
                    array = [
                        ...
                        event_time]
                    self.submissions[submission_id] = array
                    ...
                    if grade != "" and max_grade != "":
                        array = [
                            ...
                            grade]
                        self.assessments[submission_id] = array
    ...
    def postprocessing(self):
        ...
        for submission_id in self.submissions.keys():
            submission_record.append(self.submissions[submission_id])

        for assessment_id in self.assessments.keys():
            assessment_record.append(self.assessments[assessment_id])
        ...
        logger.debug("Starting logging %s quiz_questions entries to database" %
                     (len(self.quiz_question_record)))
        # Quiz_question table
        for array in self.quiz_question_record:
            question_id = array[0]
        ...
        for array in submission_record:
            ...
            # This should probably not be event_time!
        logger.debug("Finished logging %s submission entries to database" %
                     (len(submission_record)))

        # Submissions table
        sql = ("insert into "
               "assessments(assessment_id, course_learner_id, max_grade, "
               "grade) values (%s,%s,%s,%s)")
        ...
        for array in assessment_record:
            ...
            grade = array[3]
            cursor.execute(sql,
                           (assessment_id, course_learner_id, max_grade,
                            grade))
...
class Sessions(EventlogProcessor):
    ...
    def __init__(self, course_metadata_map, base_path, connection):
        super(Sessions,
              self).__init__(course_metadata_map, base_path, connection)
        self.child_parent_map = course_metadata_map["child_parent_map"]
            ...
            for i in range(len(event_logs)):
                if session_id == "":
                    if ("problem+block" in event_logs[i]["event_type"] or
                        ...
                        if question_id in self.child_parent_map.keys():
                            parent_block_id = self.child_parent_map[
                                question_id]
    ...
    def handleEvent(self, jsonObject):
        ...
        if global_learner_id != "":
            ...
            if course_learner_id in self.learner_all_event_logs:
    ...
    def postprocessing(self):
        logger.debug('Processing %s quiz session events' %
                     (len(self.quiz_sessions.keys())))
        # To compress the session event_logs
        for session_id in self.quiz_sessions.keys():
            if len(self.quiz_sessions[session_id]["time_array"]) > 1:
                start_time = ""
                end_time = ""
                updated_time_array = []

                for i in range(len(self.quiz_sessions[session_id][
                        "time_array"])):
                    if i == 0:
                        start_time = self.quiz_sessions[session_id][
                            "time_array"][i]["start_time"]
                        end_time = self.quiz_sessions[session_id][
                            "time_array"][i]["end_time"]
                    else:
                        if self.quiz_sessions[session_id]["time_array"][i]["start_time"] > end_time + datetime.timedelta(hours=0.5):
                            ...
                            if (i == len(self.quiz_sessions[session_id][
                                    "time_array"]) - 1):
                                updated_time_array.append(
                                    {"start_time": start_time,
                                     "end_time": end_time})
        ...
        logger.debug('Re-processing %s quiz session events' %
                     (len(self.quiz_sessions.keys())))
        for session_id in self.quiz_sessions.keys():
            course_learner_id = self.quiz_sessions[
                session_id]["course_learner_id"]
            for i in range(len(self.quiz_sessions[session_id]["time_array"])):
